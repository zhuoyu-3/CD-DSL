...
OUT_DIR="${OUT_DIR:-./runs/cddsl_iid_$(date +%Y%m%d_%H%M%S)}"
...
PYTHONUNBUFFERED=1 "$PYTHON_BIN" -u CDDSL.py \
  ...
  --metrics-prefix cddsl_iid \
  --save-checkpoint \
  --checkpoint-dir "$OUT_DIR/checkpoints" \
  --checkpoint-prefix cddsl_iid \
  --no-download \
  --data-style iid \
  2>&1 | tee "$OUT_DIR/logs/cddsl_iid.log"

echo
echo "CD-DSL IID run finished."
