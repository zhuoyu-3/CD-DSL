...
class StandaloneClient:
    def __init__(
        ...
        self.neighbor_best_state = self.local_best_state
...
def run_standalone(cfg: CDDSLConfig) -> Dict[str, List[float]]:
    ...
    print(
        "NO communication: no mixing matrix, no push-sum, no swarm step. "
    ...
    print(f"\nStand-alone finished in {time.time() - start:.2f} seconds.")
    ...
    if cfg.save_checkpoint or cfg.save_client_checkpoints:
        clients_count = cfg.clients
        empty_A = np.eye(clients_count, dtype=np.float64)
        empty_P = np.eye(clients_count, dtype=np.float64)
        empty_masses = np.ones(clients_count, dtype=np.float64)
    ...
    if cfg.save_checkpoint:
        ...
        save_consensus_checkpoint(
            ...
            model_kind="standalone_best_consensus",
        )
        save_consensus_checkpoint(
            final_path,
            ...
            model_kind="standalone_final_consensus",
        )
