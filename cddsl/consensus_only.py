...
from .client import round_learning_rate, train_local
...
from .mixing import push_sum_consensus
from .model import build_model
from .standalone import StandaloneClient
...
def run_consensus_only(cfg: CDDSLConfig) -> Dict[str, List[float]]:
    ...
    static_self_weight = float(np.mean(np.diag(P)))

    print("\n=================== Consensus-Only Settings ===================")
    ...
    print(
        ...
        f"extra_edges={cfg.extra_edges}, mean_self_weight={static_self_weight:.4f}"
    )
    print(
    ...
    print(
        "PURE CONSENSUS: column-stochastic push-sum only, "
        "NO adaptive Eq.(12), NO swarm step. "
    ...
    for round_id in range(cfg.rounds):
        ...
        consensus_states, masses = push_sum_consensus(
            current_states, masses, P, parameter_keys
        )
        for client_id, client in enumerate(clients):
            set_state(client.model, consensus_states[client_id])
        ...
        history["mean_self_weight"].append(static_self_weight)
        ...
        if metrics_path is not None:
            append_metrics_csv(
                metrics_path,
                {
                    ...
                    "mean_self_weight": static_self_weight,
                    "lr": float(lr_round),
                    "c0": float("nan"),
    ...
    print(f"\nConsensus-only finished in {time.time() - start:.2f} seconds.")
    ...
    if cfg.save_checkpoint:
        ...
        save_consensus_checkpoint(
            ...
            model_kind="consensus_only_best_consensus",
        )
        save_consensus_checkpoint(
            final_path,
            ...
            model_kind="consensus_only_final_consensus",
        )
